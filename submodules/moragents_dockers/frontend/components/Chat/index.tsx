...
export const Chat: FC<ChatProps> = ({
  ...
  const isMostRecentSwapMessage = useCallback(
    (message: ChatMessage) => {
      const swapsMessages = messagesData.filter(
        (message) => message.role === "swap"
      );

      if (message.role === "swap") {
        const isLastMessage =
          messagesData[messagesData.length - 1]?.content === message.content;
    ...
    >
      <Box
        ...
        sx={{
          ...
          ...(availableAgents[selectedAgent]?.requirements.connectedWallet &&
          !address
            ? {
                pointerEvents: "none",
              }
      ...
      >
        {messagesData.map((message: ChatMessage, index) => {
          console.log(`Message ${index} agentName:`, message.agentName);
          return (
            <Grid
              templateAreas={`
                "avatar name"
              ...
              <GridItem area={"message"}>
                {typeof message.content === "string" ? (
                  ...
                  <SwapForm
                    ...
                    fromMessage={
                      message.content as unknown as SwapMessagePayload
                    }
                        ...
                        },
                        {
                          onSuccess: (hash) => {
                            sendSwapStatus(
                              ...
                              .catch((error) => {
                                console.log(
                                  `Error sending swap status: ${error}`
                                );
                                onBackendError();
                              });
