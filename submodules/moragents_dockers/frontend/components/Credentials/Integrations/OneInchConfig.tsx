...
export const OneInchConfig: React.FC<OneInchConfigProps> = ({ onSave }) => {
  ...
  useEffect(() => {
    const storedUrl = localStorage.getItem("oneinch_irys_url");
    if (storedUrl) {
  ...
  const handleSave = async () => {
    try {
      ...
      localStorage.setItem("oneinch_irys_url", uploadedUrl);
      setIrysUrl(uploadedUrl);
      setNewApiKey("");
      onSave();
    } catch (error) {
      console.error("Failed to encrypt and save 1inch API key:", error);
      toast({
  ...
  const handleReset = () => {
    localStorage.removeItem("oneinch_irys_url");
    setIrysUrl("");
    setDecryptedSecret("");
  };
  ...
  return (
    <VStack spacing={6} align="stretch" className="oneinch-config-container">
      <Box>
        <Text color="white" fontSize="14px" fontWeight="500" mb={2}>
          1inch API Configuration
        </Text>
        <Text fontSize="12px" color="rgba(255, 255, 255, 0.6)">
          Enter your 1inch API key. The key will be encrypted using Lit Protocol
