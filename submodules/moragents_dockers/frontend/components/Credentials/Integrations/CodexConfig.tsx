...
export const CodexConfig: React.FC<CodexConfigProps> = ({ onSave }) => {
  ...
  useEffect(() => {
    const storedUrl = localStorage.getItem("codex_irys_url");
    if (storedUrl) {
  ...
  const handleSave = async () => {
    try {
      ...
      localStorage.setItem("codex_irys_url", uploadedUrl);
      setIrysUrl(uploadedUrl);
      setNewApiKey("");
      onSave();
    } catch (error) {
      console.error("Failed to encrypt and save Codex API key:", error);
      toast({
  ...
  const handleReset = () => {
    localStorage.removeItem("codex_irys_url");
    setIrysUrl("");
    setDecryptedSecret("");
  };
  ...
  return (
    <VStack spacing={6} align="stretch" className="codex-config-container">
      <Box>
        <Text color="white" fontSize="14px" fontWeight="500" mb={2}>
          Codex API Configuration
        </Text>
        <Text fontSize="12px" color="rgba(255, 255, 255, 0.6)">
          Enter your Codex API key. The key will be encrypted using Lit Protocol
          and stored on Irys. The decrypted secret will only be used for API
          requests and never sent to Morpheus.
        </Text>
      </Box>
