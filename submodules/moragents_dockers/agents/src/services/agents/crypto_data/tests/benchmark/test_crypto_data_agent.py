...
from src.services.agents.crypto_data.agent import CryptoDataAgent
...
async def test_get_price_success(crypto_agent, make_chat_request):
    ...
    with patch("src.services.agents.crypto_data.tools.get_coin_price_tool") as mock_price:
        mock_price.return_value = "Bitcoin price is $50,000"
        with patch("src.services.agents.crypto_data.tools.get_coingecko_id") as mock_id:
            mock_id.return_value = "bitcoin"
            with patch("src.services.agents.crypto_data.tools.get_tradingview_symbol") as mock_symbol:
...
async def test_get_market_cap_success(crypto_agent, make_chat_request):
    ...
    with patch("src.services.agents.crypto_data.tools.get_coin_market_cap_tool") as mock_mcap:
...
async def test_get_tvl_success(crypto_agent, make_chat_request):
    ...
    with patch("src.services.agents.crypto_data.tools.get_protocol_total_value_locked_tool") as mock_tvl:
...
async def test_get_floor_price_success(crypto_agent, make_chat_request):
    ...
    with patch("src.services.agents.crypto_data.tools.get_nft_floor_price_tool") as mock_floor:
