...
from src.swap_agent.src.tools import (
    search_tokens, get_token_balance, eth_to_wei, validate_swap,
    get_quote, get_token_decimals, convert_to_smallest_unit,
    convert_to_readable_unit, swap_coins, InsufficientFundsError,
    TokenNotFoundError, SwapNotPossibleError
)
...
def test_swap_coins():
    with patch('src.swap_agent.src.tools.Web3') as mock_web3, \
         patch('src.swap_agent.src.tools.validate_swap') as mock_validate, \
         patch('src.swap_agent.src.tools.get_quote') as mock_quote:
