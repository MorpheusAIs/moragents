import sys
import os
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'src', 'swap_agent', 'src')))
# The above sys/os lines are needed because the agents have identical namings (i.e. agent.py, tools.py)

import pytest
from unittest.mock import patch, MagicMock
from flask import Flask, request
from src.swap_agent.src.agent import get_response, generate_response, chat, get_messages, clear_messages, get_allowance, approve, swap
...
def test_chat(app, mock_llm):
    with app.test_request_context(json={"prompt": "Hello", "wallet_address": "0x123", "chain_id": "1"}):
        with patch('src.swap_agent.src.agent.generate_response', return_value=("This is a test response", "assistant")):
...
def test_get_allowance(app):
    with app.test_request_context(json={"tokenAddress": "0x123", "walletAddress": "0x456", "chain_id": "1"}):
...
def test_approve(app):
    with app.test_request_context(json={"tokenAddress": "0x123", "chain_id": "1", "amount": "1000"}):
        with patch('src.swap_agent.src.agent.approve_transaction', return_value={"txHash": "0x789"}):
