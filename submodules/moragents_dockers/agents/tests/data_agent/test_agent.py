import sys
import os
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'src', 'data_agent', 'src')))
# The above sys/os lines are needed because the agents have identical namings (i.e. agent.py, tools.py)

import pytest
from unittest.mock import patch, MagicMock
from src.data_agent.src.agent import get_response, generate_response, chat, get_messages, clear_messages
...
def test_chat(app, mock_llm):
    with app.test_request_context(json={"prompt": "Hello"}):
        with patch('agent.generate_response', return_value=("This is a test response", "assistant")):
